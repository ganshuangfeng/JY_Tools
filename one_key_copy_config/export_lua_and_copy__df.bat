...
set doc_dir=E:\Dongfeng\Dongfeng_doc

:: tools的路径
set tools_dir=E:\JyQipai\JyQipai_tools

:: 导出py脚本
set py_sprite=e2l_df.py

:: 服务器 文件夹路径
set server_dir_name_1=E:\Dongfeng\Dongfeng_server\skynet\game\config
set server_dir_name_2=E:\Dongfeng\Dongfeng_server_copy\skynet\game\config
set server_dir_name_3=E:\Dongfeng\Dongfeng_server_copy2\skynet\game\config
...
:: -------------------------------------------------------------------------------- 选配置类型 ↓
:: 要拷贝的文件的txt文件
set need_copy_file_txt=copy_doc_for_server_df.txt
set config_type=1
...
:: ------------------------------------------------------------------------------- 选doc版本 ↓
:: doc 要导出的文件夹
set export_dir=server
set /p version=---------请输入要导出的配置版本，输1是server;输10.27表示config_10.27 :

if %version%==1 (
	if not exist %doc_dir%\server (
		echo 不存在文件夹 %doc_dir%\server
	)
) else (
